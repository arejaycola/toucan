...
const TweetHelper = () => {
	...
	const { initialTweets } = useContext(InitialStatusContext);
	...
	useLayoutEffect(() => {
		...
		setTweetsCount(initialTweets.length);

		setEffectCount(effectCount + 1);
		/* Extract a list of just userIds to be queried */
		const userIds = initialTweets
			.map((tweet) => {
		...
		const sendUsersRequest = async () => {
			if (userIds.length > 0) {
				...
				for (let i = 0; i < initialTweets.length; i++) {
					const initialTweet = initialTweets[i];

					for (let j = 0; j < initialTweet.entities.user_mentions.length; j++) {
						const userMentions = initialTweet.entities.user_mentions[j];
						...
						for (let k = 0; k < users.length; k++) {
							...
							if (userMentions.id === user.id) {
								if (user.verified) {
									...
									initialTweet.userType = 'verified';
								} else {
									userMentions.userType = 'unverified';
									userMentions.verified = false;
									initialTweet.userType = unverifiedUserMentions['user-' + user.id] = 'unverified';
								}
							}
						}
					}
				...
				initialTweets.map((initialTweet) => {
					let tempMoment = moment(new Date(initialTweet.created_at));

					initialTweet.entities.user_mentions.map((userMention) => {
